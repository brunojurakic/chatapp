...
public class ChatService {
    ...
    public List<ChatMessageDTO> getRecentMessages(UUID friendshipId, int limit) {
        ...
        return msgs.stream().map(m -> new ChatMessageDTO(
            m.getId(),
            f.getId(),
            m.getSender().getId(),
            m.getSender().getDisplayName() != null ? m.getSender().getDisplayName() : m.getSender().getName(),
    ...
    public ChatMessageDTO saveMessage(UUID friendshipId, User sender, String content) {
        ...
        if (!f.getUserA().getId().equals(sender.getId()) && !f.getUserB().getId().equals(sender.getId())) {
        ...
        return new ChatMessageDTO(
            ...
            saved.getSender().getDisplayName() != null ? saved.getSender().getDisplayName() : saved.getSender().getName(),
